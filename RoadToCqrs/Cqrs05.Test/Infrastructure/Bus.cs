...
namespace Cqrs02.Test.Infrastructure
{
    public class Bus 
    {
        ...
        public void Send(object message,DateTime? delay=null)
        {
            if (delay == null)
            {
            ...
            {
                var realDely = delay.Value - DateTime.Now;
                System.Threading.Tasks.Task.Delay(realDely).
                    ContinueWith(_ => InternalSend(message));
            }
        }

        private void InternalSend(object message)
        {
            lock (_lock)
            {
